...
t=[0.9:dt:2.1];
t_idx=find(t>=1 & t<=2);tt=[1:dt/10:1+dt 1+dt*2:dt:2];
step=20;
t0=t(1:step:end);
...
u_star=zeros(length(tt), nx+1, ny+2, nz+2); v_star=zeros(length(tt), nx+2, ny+1, nz+2);
w_star=zeros(length(tt), nx+2, ny+2, nz+1); dp=zeros(length(tt), nx+2, ny+2, nz+2);
...
plot(t0, squeeze(dp0(1:step:end,5,6,20)),'-s', tt, squeeze(dp(:,5,6,20)),'x');
%%
t_start=1; t_end=1.3;
