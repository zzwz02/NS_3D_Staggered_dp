    ...
    do t_step=0,time_length
        ...
        if (t_step==0) then
            ...
            RHS_poisson_internal = diff_old(u_star(:,2:ubound(u_star,2)-1,2:ubound(u_star,3)-1),1,1)/dx
        ...
        if (tGet>=0 .and. save_output) then
            ...
            if (mod(abs(tGet), 1.0d0) < dt0/sub_tstep/100.0d0 .or. abs(tGet-0.9)< dt0/sub_tstep/100.0d0) then
