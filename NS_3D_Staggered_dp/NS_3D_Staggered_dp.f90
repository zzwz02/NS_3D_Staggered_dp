    ...
    real(8), parameter :: Re=1.0d0, nu=0.005d0, t_start=-1.0d0, t_end=-0.995d0, dt0=0.0005d0
    ...
    real(8), dimension (time_length), parameter :: time_array=[(t_start+dt0*i, i=1, time_length)]
    ...
    real(8) :: rhs_x_previous(nx+1,ny+2,nz+2)=0, rhs_y_previous(nx+2,ny+1,nz+2)=0, rhs_z_previous(nx+2,ny+2,nz+1)=0, dp_vec(nxp*nyp*nzp)=0, dp_dft(nxp,nyp,nzp)=0
    ...
    real(8), dimension (:,:,:), allocatable :: test1, test2, test3, test4, test5, test6
    ...
    do t_step=1,time_length
        ...
        dp_dft(2:nxp-1,2:nyp-1,2:nzp-1)=dft_out_r
        dp_dft(1,:,:)=dp_dft(nxp-1,:,:); dp_dft(nxp,:,:)=dp_dft(2,:,:)
        dp_dft(:,1,:)=dp_dft(:,nxp-1,:); dp_dft(:,nxp,:)=dp_dft(:,2,:)
        dp_dft(:,:,1)=dp_dft(:,:,nxp-1); dp_dft(:,:,nxp)=dp_dft(:,:,2)
        ...
        print '("Complete: ", (f6.4), " second")', (c02-c01)/system_clock_rate
    end do

    status = DftiFreeDescriptor(hand_f)
    status = DftiFreeDescriptor(hand_b)

    !temp3=diff_test(p, 1, 1)
    !temp4=diff_test(p, 1, 2)
    !temp2=diff(p, 1, 1)
    ...
    !    CALL SYSTEM_CLOCK(c02)
    !    do i=1,10
    !        temp2=diff_old(p, 1, 1)
    !    end do
    !    CALL SYSTEM_CLOCK(c02)
    !    print '(" timing 2: ", (f6.2), " second")', (c02-c01)/system_clock_rate
    !
    !    temp3=diff(p, 1, 2)
    !    temp4=diff_old(p, 1, 2)
    !    temp5=diff(p, 1, 3)
    !    temp6=diff_old(p, 1, 3)
    !
    !    print *, 'diff'
    !    print *, all(temp1==temp2)
    !    print *, all(temp3==temp4)
    !    print *, all(temp5==temp6)
    !    
    !    print *, 'diff2'
    !    temp1=diff2(u,1,1);
    !    temp2=diff2_old(u,1,1);
    !    print *, all(temp1==temp2)
    !    temp1=diff2(u,1,0);
    !    temp2=diff2_old(u,1,0);
    !    print *, all(temp1==temp2)
    !    temp1=diff2(u,2,1);
    !    temp2=diff2_old(u,2,1);
    !    print *, all(temp1==temp2)
    !    temp1=diff2(u,2,0);
    !    temp2=diff2_old(u,2,0);
    !    print *, all(temp1==temp2)
    !    temp1=diff2(u,3,1);
    !    temp2=diff2_old(u,3,1);
    !    print *, all(temp1==temp2)
    !    temp1=diff2(u,3,0);
    !    temp2=diff2_old(u,3,0);
    !    print *, all(temp1==temp2)
    !    
    !    print *, 'avg'
    !    temp1=avg(u,1,1);
